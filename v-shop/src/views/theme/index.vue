...
const active = ref(-1);
// Check if current theme is rainbow
const isCurrentRainbow = unref(list).some(
  (item) => item.isRainbow && item.colors.primary === unref(theme).colors.primary,
);
if (isCurrentRainbow) {
  active.value = unref(list).findIndex((item) => item.isRainbow);
} else {
...
function onThemeColorChange(index: number) {
  active.value = index;
  const item = unref(list)[unref(active)];
  const { colors, isRainbow } = item;
  appStore.updateTheme({ colors, isRainbow });
}
