...
const { hasLogin, isOmnipotent, goLogin, goPage } = usePage();
...
// 万能账户模式下点击头像区域
function onOmnipotentAvatarClick() {
  // 什么也不做，保持当前状态
  ElMessage.info('万能账户模式');
}
  ...
  <div class="page-container">
    ...
    <main class="main-content">
      <div class="white-container">
        <div class="header">
          <template v-if="isOmnipotent">
            <div class="header-inner">
              <div class="header-tag">万能账户</div>
              <van-image
                class="header-avatar"
                :src="userInfo.avatarUrl || assets.avatar"
                alt=""
                @click.stop="onOmnipotentAvatarClick"
              />
              <div class="header-info">
                <div class="header-nick header-nick-logged van-ellipsis mb10">
                  {{ userInfo.nick || `万能管理员` }}
                </div>
                <div class="header-sub">
                  <span class="header-sub-item">ID {{ userInfo.id }}</span>
                  <span class="header-sub-item-separate">|</span>
                  <span class="header-sub-item">调试模式</span>
                </div>
              </div>
            </div>
          </template>
          <template v-else>
            ...
            <div v-else class="header-inner" @click="goLogin">
              ...
              <div class="header-info">
                <div class="header-nick header-nick-logout">登录/注册</div>
              </div>
            </div>
          </template>
          <div class="header-bg">
