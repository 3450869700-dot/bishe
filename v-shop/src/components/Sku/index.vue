...
/**
 * SKU列表的索引映射，用于O(1)时间复杂度查找
 * key: propertyChildIds, value: skuItem
 */
const skuListIndex = computed(() => {
  const index: Record<string, Recordable> = {};
  if (props.sku.skuList) {
    props.sku.skuList.forEach((item) => {
      index[item.propertyChildIds] = item;
    });
  }
  return index;
});
...
const skuValue = computed((): Recordable | undefined => {
  if (unref(hasSku)) {
    ...
    // 使用索引查找，O(1)时间复杂度
    return unref(skuListIndex)[propertyChildIds];
  }
...
/**
 * 已选属性ID的索引集合，用于O(1)时间复杂度查找
 */
const selectedPropIdsIndex = computed(() => {
  const index: Set<string | number> = new Set();
  props.initialSku.selectedPropList.forEach((item) => {
    index.add(item.id);
  });
  return index;
});
...
const selectedPropTitle = computed(() => {
  if (unref(hasSku)) {
    ...
    return unref(skuValue)
      ...
      : `请选择 ${propList.reduce(
          (acc, cur) => `${acc}${unref(selectedPropIdsIndex).has(cur.id) ? '' : cur.name}`,
          '',
        )}`;
  } else {
    return '';
...
/**
 * 属性子项索引映射
 * key: `${propId}:${childId}`, value: { propId, propName, childId, childName }
 */
const propChildIndex = computed(() => {
  const index: Record<string, { propId: number; propName: string; childId: number; childName: string }> = {};
  props.sku.propList.forEach((prop) => {
    prop.childsCurGoods.forEach((child: Recordable) => {
      index[`${prop.id}:${child.id}`] = {
...
function onPropClicked(index: number, i: number) {
  ...
  // 使用索引构建选中列表，避免嵌套遍历 O(n*m) -> O(n)
  const selectedPropList: ISelectedPropItem[] = [];
  Object.keys(props.initialSku.selectedProps).forEach((propId) => {
    const childId = props.initialSku.selectedProps[propId];
    if (childId && childId !== 0) {
      const key = `${propId}:${childId}`;
      const indexedItem = unref(propChildIndex)[key];
      if (indexedItem) {
        selectedPropList.push({
          id: indexedItem.propId,
          name: indexedItem.propName,
          childId: indexedItem.childId,
          childName: indexedItem.childName,
          propIds: key,
        });
      }
    }
